...
struct DefectInfoItem {
   /* DefectInfoItem(TRACK_INDEX _trackIndex,
    ...
    DefectInfoItem(TRACK_INDEX _trackIndex,
        ...
        , pixelValue(_pixelValue)
        , mmValue(_mmValue)
        , minValue(_mmValue)      // НОВОЕ: инициализируем как текущее
        , maxValue(_mmValue)      // НОВОЕ: инициализируем как текущее
        , isPass(_isPass)
        , measurementCount(1)     // НОВОЕ: счетчик измерений
    {
    }
    ...
    double pixelValue;
    double mmValue;
    double minValue;    // НОВОЕ: минимальное значение
    double maxValue;    // НОВОЕ: максимальное значение
    bool isPass;
    int measurementCount; // НОВОЕ: сколько раз измеряли
};
