...
void ReportVM::AddInspectionDataWithStats(InspectionInfoItem data, QList<DefectInfoItem> defectInfos)
{
    ...
    // Преобразуем defectInfos в QVariantList для QML
    QVariantList statsList;
    for (const auto& defect : defectInfos) {
        QVariantMap stat;
        stat["parameter"] = defect.parameter;
        stat["defect"] = defect.defect;
        stat["minValue"] = defect.minValue;
        stat["maxValue"] = defect.maxValue;
        stat["mmValue"] = defect.mmValue;  // текущее значение
        stat["isPass"] = defect.isPass;
        stat["measurementCount"] = defect.measurementCount;
        statsList.append(stat);
    }

    emit addInspectionDataWithStats_Signal(data, statsList);
}
...
void ReportVM::AddInspectionData(InspectionInfoItem data)
{
    ...
    qDebug() << "[ReportVM] AddInspectionData called (OLD VERSION)";
    m_listInspectionInfor.append(data);
