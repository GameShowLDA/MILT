...
void MasterApp::inspectFinish_InspectionCore_Slots(OutputBackendInfo &result)
{
    try {
        ...
        if(result.function == BACKEND_FUNCTION::INSPECT_DONE)
        {
            ...
            qDebug() << "[DEBUG] ======= STATISTICS DATA =======";
            qDebug() << "[DEBUG] Total defectInfos count:" << result.defectInfos.count();
            for (int i = 0; i < result.defectInfos.count(); i++) {
                const auto& defect = result.defectInfos[i];
                qDebug() << "[DEBUG]" << i << "Param:" << defect.parameter
                         << "Value:" << defect.mmValue
                         << "Min:" << defect.minValue
                         << "Max:" << defect.maxValue
                         << "Count:" << defect.measurementCount
                         << "Pass:" << defect.isPass;
            }
            qDebug() << "[DEBUG] ===============================";
            ...
            QList<DefectInfoItem> statsDefectInfos;

            // Проверяем, есть ли статистика
            for (const auto& defect : result.defectInfos) {
                // Если есть min/max значения или количество измерений > 1
                if (defect.measurementCount > 1 || defect.minValue != defect.maxValue) {
                    hasStatistics = true;
                    statsDefectInfos.append(defect);

                    qDebug() << "[DEBUG] Found statistics for:" << defect.parameter
                             << "min:" << defect.minValue
                             << "max:" << defect.maxValue
                             << "count:" << defect.measurementCount;
                }
            ...
            if (hasStatistics && !statsDefectInfos.isEmpty()) {
                qDebug() << "[DEBUG] Using statistics version, defectInfos count:" << statsDefectInfos.count();
                ReportVM::GetInstance()->AddInspectionDataWithStats(inspectionInfoItem, statsDefectInfos);
            } else {
                qDebug() << "[DEBUG] Using old version (no statistics)";
                ReportVM::GetInstance()->AddInspectionData(inspectionInfoItem);
            }
